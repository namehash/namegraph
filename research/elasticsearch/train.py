...
if __name__ == '__main__':
    ...
    parser.add_argument('--do_train', help='train the models', default=False, action='store_true')
    parser.add_argument('--do_test', help='create the test reports', default=False, action='store_true')
    ...
    def report_dir(args, part):
        return f"{args.reports}/{args.experiment}-{args.metric}/{part}/"

    def report_path(args, model_id, part):
        return f"{report_dir(args, part)}{model_id}.report"    
    ...
    if(args.do_test):
        subprocess.run(["mkdir", "-p", report_dir(args, "test")], capture_output=True)
        subprocess.run(["mkdir", "-p", report_dir(args, "train")], capture_output=True)

        params = copy.copy(GLOBAL_PARAMS)
        params += ["-test", args.test]
        params += ["-metric2T", args.metric]
        params += ["-idv", report_path(args, "baseline", "test")]
        params += ["-gmax", "5"]
        result = subprocess.run(params, capture_output=True)

        params = copy.copy(GLOBAL_PARAMS)
        params += ["-test", args.train]
        params += ["-metric2T", args.metric]
        params += ["-idv", report_path(args, "baseline", "train")]
        params += ["-gmax", "5"]
        result = subprocess.run(params, capture_output=True)
        ...
        for model_id in tqdm.tqdm(range(MODEL_RANGE)):
            ...
            params += ["-test", args.test]
            params += ["-metric2T", args.metric]
            params += ["-idv", report_path(args, model_id, "test")]
            ...
            params += ["-test", args.train]
            params += ["-metric2T", args.metric]
            params += ["-idv", report_path(args, model_id, "train")]
            params += ["-load", model_path(args, model_id)]
            params += ["-gmax", "5"]
        ...
        params += ["-all", report_dir(args, "test")]
        params += ["-base", "baseline.report"]

        result = subprocess.run(params, capture_output=True)
        with open(f"{args.reports}/{args.experiment}-{args.metric}-test-analysis.txt", "w") as output:
        ...
        params = ["java", "-cp", "ranklib-ranklib-2.10.1/target/ranklib-2.10.1.jar", "ciir.umass.edu.eval.Analyzer"]
        params += ["-all", report_dir(args, "train")]
        params += ["-base", "baseline.report"]

        result = subprocess.run(params, capture_output=True)
        with open(f"{args.reports}/{args.experiment}-{args.metric}-train-analysis.txt", "w") as output:
            for line in (str(result.stderr).split("\\n")):
                if(re.match("maj", line)):
                    continue
                output.write(re.sub(r"\\t", "\t", line)[6:] + "\n")
